...
<<<<<<< HEAD
        ...
        for idx, unit in enumerate(doc.units, start=1):
            page_nums = unit.metadata.page_numbers or []
            accuracy = unit.metadata.custom.get('accuracy')
            console.print(f"\n  [cyan]TableUnit #{idx}:[/cyan]")
            console.print(f"    Unit ID: {unit.unit_id[:16]}...")
            console.print(f"    Page: {page_nums}")
            console.print(f"    Unit Type: {unit.unit_type}")
            if accuracy:
                console.print(f"    Accuracy: {accuracy:.2f}%")
            console.print(f"    Has DataFrame: {unit.df is not None}")
            if unit.df is not None:
                console.print(f"    DataFrame Shape: {unit.df.shape}")
    
    # Show first TableUnit preview
    if doc.units:
        console.print(f"\n[bold]First TableUnit Preview:[/bold]")
        first_unit = doc.units[0]
