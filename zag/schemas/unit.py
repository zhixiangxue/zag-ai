...
class TableUnit(BaseUnit):
    ...
    Attributes:
        ...
        embedding_content: Natural language description for embedding (generated by TableEnricher)
        df: Table data as pandas DataFrame (mandatory, generated by TableParser)
        caption: Table caption or title (generated by TableEnricher)
    
    Design:
        - content: Original table format (for LLM and display)
        - embedding_content: Detailed NL description (for vector search)
        - df: Structured data as DataFrame (for analysis, filtering, transformation)
        - caption: Intelligent title based on content and context
        
        When embedding_content is None, use content for embedding.
        
    Processing workflow:
        1. TableParser extracts from TextUnit → creates TableUnit with df
        2. TableEnricher generates embedding_content and caption
        3. Store to vector database with all fields populated
    ...
    def __add__(self, other: 'TableUnit') -> 'TableUnit':
        ...
        - df set to None (DataFrame merging not yet implemented)
