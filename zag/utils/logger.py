...
Advanced Usage:
    ...
    from zag.utils import logger, set_level
    
    set_level("DEBUG")  # Switch to debug mode
    logger.debug("This will now be visible")
    
    # Context binding for module identification
    logger = logger.bind(module="VectorStore")
    logger.info("Searching vectors...")  # Output: ... | VectorStore | Searching vectors...
...
LOG_FILE = os.getenv("LOG_FILE", "logs/zag.log")
...
def set_level(level: str):
    ...
    Example:
        from zag.utils import set_level
        set_level("DEBUG")  # Switch to debug mode
    """
    level = level.upper()
